...
table(all.samples$tissue) # sangre: 2142 + DNA: 207
...
dim(merge_samples_gcat) # 9167  191
merge_samples = NULL
...
samples_information = all.samples %>% filter(xor(tissue == "Sangre", tissue == "DNA")) %>% dplyr::select(analysisId, sampleId, version_panel, individualProgenyId, GeneCascadesName)
...
dim(samples_information) # We loose two individuals: 2447    6
...
head(cascade_discard)
dim(cascade_discard)#292   8
table(cascade_clasification$cascades.polides)
table(cascade_discard$X8)
...
cascade_clasification = cascade_clasification %>% dplyr::select(individualProgenyId, cascades.polides)
...
length(which(is.na(samples_information$cascades.polides))) # 0
...
dim(final_merge_samples) # 35893  2431
dim(samples_information) # 1946    7
# In common samples between them: 1581
length(which(colnames(final_merge_samples) %in% samples_information$sampleId)) # 1581

# Take the selected genotypes
selected_merge_samples = final_merge_samples[,which(colnames(final_merge_samples) %in% samples_information$sampleId)] # , with=F

# Now add the 6 first columns with the variables information
selected_merge_samples = cbind(final_merge_samples[,1:6], selected_merge_samples)

dim(selected_merge_samples) # 35893  1587 (1581+6) Ok

#fwrite(selected_merge_samples, "selected_merge_samples.txt")

selected_merge_samples = fread("selected_merge_samples.txt")
