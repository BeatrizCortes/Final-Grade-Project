...
for (i in 1:nrow(samples_information)){
  if ((samples_information[i,]$Type == "Control") && (is.na(samples_information[i,]$cascades.polides))){
    samples_information[i,]$cascades.polides = "Control"
  }
}

sum(is.na(samples_information$cascades.polides))
#261 Cases without clasification

# We want to filter the data without clasification
dim(samples_information)
table(samples_information$cascades.polides)
samples_information = samples_information[samples_information$cascades.polides != ""]
dim(samples_information) # 2195. It is 2456 (all the samples)-261 (the samples without clasification) 

# Let's check the not found samples
head(which(is.na(samples_information$cascades.polides))) # 15  94 229 293 294 295
